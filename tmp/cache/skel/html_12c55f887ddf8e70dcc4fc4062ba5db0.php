 ...
 * Date :      Wed, 03 Jun 2015 10:33:05 GMT
...
quete_condition_postdates('documents.date_publication',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_12c55f887ddf8e70dcc4fc4062ba5db0','_doc2',69,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
interdire_scripts((match($Pile[$SP]['extension'],'^(gif|jpg|png)$') ? (	filtrer('image_graver',filtrer('image_reduire',
	((($recurs=(isset($Pile[0]['recurs'])?$Pile[0]['recurs']:0))>=5)? '' :
	recuperer_fond('modeles/emb', array('lang' => $GLOBALS["spip_lang"] ,'id_document'=>$Pile[$SP]['id_document'],'id'=>$Pile[$SP]['id_document'],'recurs'=>(++$recurs)), array('compil'=>array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_12c55f887ddf8e70dcc4fc4062ba5db0','_doc2',71,$GLOBALS['spip_lang']), 'trim'=>true), ''))
,'300')) .
	'
				'):(	quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), vider_url(urlencode_1738(generer_url_entite($Pile[$SP]['id_document'], 'document', '', '', true))), '', '', 0, 0, '') .
	'
...
function BOUCLE_forums_filshtml_12c55f887ddf8e70dcc4fc4062ba5db0(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_12c55f887ddf8e70dcc4fc4062ba5db0','_forums_fils',48,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
(($Pile[$SP]['id_auteur'])  ?
	...
	((($recurs=(isset($Pile[0]['recurs'])?$Pile[0]['recurs']:0))>=5)? '' :
	recuperer_fond('modeles/auteur_nom', array('id_auteur' => $Pile[$SP]['id_auteur'] ,'lang' => $GLOBALS["spip_lang"] ,'id_forum'=>$Pile[$SP]['id_forum'],'id'=>$Pile[$SP]['id_forum'],'recurs'=>(++$recurs)), array('compil'=>array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_12c55f887ddf8e70dcc4fc4062ba5db0','_forums_fils',53,$GLOBALS['spip_lang']), 'trim'=>true), ''))
 .
	'</a>
						')) :
		'') .
...
// Temps de compilation total: 33.172 ms
//
