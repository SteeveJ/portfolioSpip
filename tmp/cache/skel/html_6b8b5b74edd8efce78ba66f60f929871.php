 ...
 * Compile :   Thu, 04 Jun 2015 12:21:54 GMT
 * Boucles :   _liens, _galerie
 */ 

function BOUCLE_lienshtml_6b8b5b74edd8efce78ba66f60f929871(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/medias/prive/squelettes/inclure/mediatheque-galerie.html','html_6b8b5b74edd8efce78ba66f60f929871','_liens',68,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'
			' .
lien_objet($Pile[$SP]['id_objet'],interdire_scripts($Pile[$SP]['objet'])) .
'
			');
...
function BOUCLE_galeriehtml_6b8b5b74edd8efce78ba66f60f929871(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  array (
	...
	$senstri = '';
	$tri = (($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'');
	if ($tri){
		$senstri = ((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1);
	...
	$command['where'] = 
			...
			array('REGEXP', 'documents.statut', sql_quote(interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'statut', null), '.*'),true)))), (@$Pile[0]["orphelins"])?'documents.id_document NOT IN (
SELECT DISTINCT id_document
FROM spip_documents_liens as oooo)':'', 
			array('REGEXP', 'documents.titre', sql_quote(interdire_scripts((entites_html(table_valeur(@$Pile[0], (string)'sanstitre', null),true) ? '^$':'.*')))), 
			array('REGEXP', 'documents.id_vignette', sql_quote(interdire_scripts((entites_html(table_valeur(@$Pile[0], (string)'sansvignette', null),true) ? '^0':'.*')))));
	if (defined("_BOUCLE_PROFILER")) $timer = time()+microtime();
	$t0 = "";
	// REQUETE
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/medias/prive/squelettes/inclure/mediatheque-galerie.html','html_6b8b5b74edd8efce78ba66f60f929871','_galerie',4,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
(($t1 = strval(interdire_scripts(((($Pile[$SP]['media'] == 'image')) ?' ' :''))))!=='' ?
		($t1 . (($t2 = strval(inserer_attribut(filtrer('image_graver',filtrer('image_reduire',quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), '', '', 'apercu', 250, 100, ''), '250', '100')),'alt',interdire_scripts(texte_backend(couper(typo(supprimer_numero($Pile[$SP]['titre']), "TYPO", $connect, $Pile[0]),'60'))))))!=='' ?
			((	'
			<a href="' .
...
'
		' .
(($t1 = strval(interdire_scripts(((($Pile[$SP]['media'] == 'image')) ?'' :' '))))!=='' ?
		($t1 . (($t2 = strval(inserer_attribut(filtrer('image_graver',filtrer('image_reduire',quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), '', '', 'auto', 250, 100, ''), '250', '100')),'alt',interdire_scripts(texte_backend(couper(typo(supprimer_numero($Pile[$SP]['titre']), "TYPO", $connect, $Pile[0]),'60'))))))!=='' ?
...
'
			' .
(($t1 = BOUCLE_lienshtml_6b8b5b74edd8efce78ba66f60f929871($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
			' .
...
'
		...
		<td class="descriptif">
			' .
(($t1 = strval(interdire_scripts(((($a = typo(supprimer_numero($Pile[$SP]['titre']), "TYPO", $connect, $Pile[0])) OR (is_string($a) AND strlen($a))) ? $a : concat('<i>',$l3,'</i>')))))!=='' ?
...
// Temps de compilation total: 41.095 ms
//

function html_6b8b5b74edd8efce78ba66f60f929871($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
...
'">
	' .
(($t1 = BOUCLE_galeriehtml_6b8b5b74edd8efce78ba66f60f929871($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
	...
	<div class=\'pagination tri\'>
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('>',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','>')-1):'>'),interdire_scripts(filtre_balise_img_dist(chemin_image('tri-asc-16.png'),_T('public|spip|ecrire:par_tri_croissant'))),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','>')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='>'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('<',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','<')-1):'<'),interdire_scripts(filtre_balise_img_dist(chemin_image('tri-desc-16.png'),_T('public|spip|ecrire:par_tri_decroissant'))),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','<')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='<'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('id_document',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','id_document')-1):'id_document'),_T('medias:par_id'),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','id_document')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='id_document'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('date',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','date')-1):'date'),_T('medias:par_date'),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','date')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='date'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('taille',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','taille')-1):'taille'),_T('medias:par_taille'),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','taille')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='taille'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('largeur',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','largeur')-1):'largeur'),_T('medias:par_largeur'),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','largeur')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='largeur'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('hauteur',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','hauteur')-1):'hauteur'),_T('medias:par_hauteur'),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','hauteur')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='hauteur'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
		' .
		(($t3 = strval(lien_ou_expose(parametre_url(self(),(($s=in_array('titre',array('>','<')))?'sens':'tri').'_galerie',$s?(strpos('< >','titre')-1):'titre'),_T('medias:par_titre'),$s?(((intval($t=(isset($Pile[0]['sens'.'_galerie']))?$Pile[0]['sens'.'_galerie']:(is_array($s='inverse')?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','titre')-1)):((($t=(isset($Pile[0]['tri'.'_galerie']))?$Pile[0]['tri'.'_galerie']:'id_document')?tri_protege_champ($t):'')=='titre'))))!=='' ?
				($t3 . ' |') :
				'') .
		'
...
');
	...
	return analyse_resultat_skel('html_6b8b5b74edd8efce78ba66f60f929871', $Cache, $page, '../plugins-dist/medias/prive/squelettes/inclure/mediatheque-galerie.html');
}
?>