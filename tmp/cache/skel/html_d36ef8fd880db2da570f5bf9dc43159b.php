 ...
 * Date :      Wed, 03 Jun 2015 10:33:21 GMT
 * Compile :   Thu, 04 Jun 2015 12:31:11 GMT
 * Boucles :   _type_doc, _compte, _docslies
 */ 

function BOUCLE_type_dochtml_d36ef8fd880db2da570f5bf9dc43159b(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/medias/modeles/document_case.html','html_d36ef8fd880db2da570f5bf9dc43159b','_type_doc',25,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
function BOUCLE_comptehtml_d36ef8fd880db2da570f5bf9dc43159b(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/medias/modeles/document_case.html','html_d36ef8fd880db2da570f5bf9dc43159b','_compte',40,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
function BOUCLE_docslieshtml_d36ef8fd880db2da570f5bf9dc43159b(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/medias/modeles/document_case.html','html_d36ef8fd880db2da570f5bf9dc43159b','_docslies',8,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'
			' .
BOUCLE_type_dochtml_d36ef8fd880db2da570f5bf9dc43159b($Cache, $Pile, $doublons, $Numrows, $SP) .
'
				' .
...
'
			<div class="actions">
				' .
BOUCLE_comptehtml_d36ef8fd880db2da570f5bf9dc43159b($Cache, $Pile, $doublons, $Numrows, $SP)
. (	'
				' .
	...
	'
				' .
	(((($Pile[$SP]['id_objet'] < '0')) OR (invalideur_session($Cache, ((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('modifier', interdire_scripts(invalideur_session($Cache, $Pile[$SP]['objet'])), invalideur_session($Cache, $Pile[$SP]['id_objet']))?" ":""))))  ?
			(' ' . (	bouton_action($l9,invalideur_session($Cache, generer_action_auteur('dissocier_document',(	invalideur_session($Cache, $Pile[$SP]['id_objet']) .
				'-' .
				interdire_scripts(invalideur_session($Cache, $Pile[$SP]['objet'])) .
				'-' .
...
'
				' .
pipeline( 'document_desc_actions' , array('args' => array('id_document' => $Pile[$SP]['id_document'], 'position' => 'document_case'), 'data' => '') ) .
'
			</div>
			' .
...
// Temps de compilation total: 50.840 ms
//

function html_d36ef8fd880db2da570f5bf9dc43159b($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
...
'
' .
BOUCLE_docslieshtml_d36ef8fd880db2da570f5bf9dc43159b($Cache, $Pile, $doublons, $Numrows, $SP) .
'
');

	return analyse_resultat_skel('html_d36ef8fd880db2da570f5bf9dc43159b', $Cache, $page, '../plugins-dist/medias/modeles/document_case.html');
}
?>