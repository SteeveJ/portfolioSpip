...
// Temps de compilation total: 14.682 ms
//
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('formulaires/inc-login_forum') . ', array_merge('.var_export($Pile[0],1).',array(\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . ')), array("compil"=>array(\'plugins-dist/forum/formulaires/forum.html\',\'html_173150797f5c3f00b67ea7eb0634dd61\',\'\',22,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
...
' .
		interdire_scripts((((include_spip('inc/config')?lire_config('forums_urlref',null,false):'') != 'non') ? (	'<ul><li class=\'fieldset\'>
  <fieldset>
  	<legend>' .
  	...
  	<p class=\'explication\'>' .
			...
			'</p>
  	<ul>
  	<li class=\'editer saisie_nom_site' .
        ...
        <label for="nom_site">' .
			...
			'</label>
      	' .
			(($t4 = strval(table_valeur(table_valeur(@$Pile[0], (string)'erreurs', null),'nom_site')))!=='' ?
					('<span class=\'erreur_message\'>' . $t4 . '</span>') :
					'') .
			'
  	...
  	<li class=\'editer saisie_url_site' .
      ...
      <label for="url_site">' .
			...
			'</label>
    	' .
			(($t4 = strval(table_valeur(table_valeur(@$Pile[0], (string)'erreurs', null),'url_site')))!=='' ?
					('<span class=\'erreur_message\'>' . $t4 . '</span>') :
					'') .
			'
