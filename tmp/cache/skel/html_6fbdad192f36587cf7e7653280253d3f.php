...
(($t1 = strval(interdire_scripts(((table_valeur(safehtml(table_valeur($Pile[$SP]['valeur'], 'field')),'id_rubrique')) ?' ' :''))))!=='' ?
		($t1 . (	'
					' .
	vide($Pile['vars'][$_zzz=(string)'fond'] = concat('prive/squelettes/inclure/plan-',interdire_scripts(safehtml(table_valeur($Pile[$SP]['valeur'], 'table_objet'))))) .
...
?'.'>
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_objets @ ../prive/squelettes/inclure/plan-rubriques.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}
...
?'.'>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_rubriques @ ../prive/squelettes/inclure/plan-rubriques.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../prive/squelettes/inclure/plan-rubriques.html
// Temps de compilation total: 7.595 ms
//
...
function html_6fbdad192f36587cf7e7653280253d3f($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
	...
	$connect = '';
	$page = (
vide($Pile['vars'][$_zzz=(string)'nomp'] = (	'rubrub' .
	@$Pile[0]['id_rubrique'])) .
vide($Pile['vars'][$_zzz=(string)'nb'] = '50') .
