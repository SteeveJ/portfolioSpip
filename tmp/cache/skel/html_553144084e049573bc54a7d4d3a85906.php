...
function BOUCLE_articlesyndichtml_553144084e049573bc54a7d4d3a85906(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/sites/prive/objets/liste/syndic.html','html_553144084e049573bc54a7d4d3a85906','_articlesyndic',31,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
function BOUCLE_liste_siteshtml_553144084e049573bc54a7d4d3a85906(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$senstri = '';
	$tri = (($t=(isset($Pile[0]['tri'.'_liste_sites']))?$Pile[0]['tri'.'_liste_sites']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date'),true)))?tri_protege_champ($t):'');
	if ($tri){
		$senstri = ((intval($t=(isset($Pile[0]['sens'.'_liste_sites']))?$Pile[0]['sens'.'_liste_sites']:(is_array($s=table_valeur($Pile["vars"], (string)'defaut_tri', null))?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_liste_sites']))?$Pile[0]['tri'.'_liste_sites']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date'),true)))?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1);
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/sites/prive/objets/liste/syndic.html','html_553144084e049573bc54a7d4d3a85906','_liste_sites',7,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
((!is_array($l = quete_logo('id_syndic', 'ON', $Pile[$SP]['id_syndic'],'', 0))) ? '':
 ("<img class=\"spip_logos\" alt=\"\" src=\"$l[0]\"" . $l[2] .  ($l[1] ? " onmouseover=\"this.src='$l[1]'\" onmouseout=\"this.src='$l[0]'\"" : "") . ' />')),'26','20')) .
'
				<a href="' .
...
(($t1 = strval(interdire_scripts(((match($Pile[$SP]['syndication'],'off|sus')) ?' ' :''))))!=='' ?
	...
	'">' .
	(($t2 = strval(interdire_scripts(inserer_attribut(filtre_balise_img_dist(find_in_path('puce-orange-anim.gif')),'alt',$l3))))!=='' ?
			($t2 . $l4) :
			'') .
	'</span>
			')) :
		'') .
BOUCLE_articlesyndichtml_553144084e049573bc54a7d4d3a85906($Cache, $Pile, $doublons, $Numrows, $SP)
...
// Temps de compilation total: 40.379 ms
//

function html_553144084e049573bc54a7d4d3a85906($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
...
') :
		'') .
(($t1 = BOUCLE_liste_siteshtml_553144084e049573bc54a7d4d3a85906($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
' .
		(($t3 = strval(interdire_scripts(sinon(table_valeur(@$Pile[0], (string)'titre', null), singulier_ou_pluriel((isset($Numrows['_liste_sites']['grand_total'])
			? $Numrows['_liste_sites']['grand_total'] : $Numrows['_liste_sites']['total']),'info_1_site','info_nb_sites')))))!=='' ?
	...
	<thead>
		...
		lien_ou_expose(parametre_url(self(),(($s=in_array('statut',array('>','<')))?'sens':'tri').'_liste_sites',$s?(strpos('< >','statut')-1):'statut'),(	'<span title="' .
...
');
	...
	return analyse_resultat_skel('html_553144084e049573bc54a7d4d3a85906', $Cache, $page, '../plugins-dist/sites/prive/objets/liste/syndic.html');
}
?>